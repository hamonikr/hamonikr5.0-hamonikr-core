...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            settings = page.add_section(_("Mouse Wheel Speed"))

            box = SettingsWidget()
            widget = Gtk.Button.new_with_label(_("Mouse Wheel Speed Setting"))
            widget.connect("clicked", self.wheel_change_button_clicked)
            box.pack_start(widget, True, True, 0)
            settings.add_row(box)
    ...
    def wheel_change_button_clicked(self, button):
        subprocess.call(["wheelspeed"])
